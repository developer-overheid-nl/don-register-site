        ...
        <defs>
          <linearGradient id="gradient">
            {colors.gradient && colors.gradient.map((color, index) => (
              <stop key={`grd-${index}`} offset={`${index * 100 / (colors.gradient.length - 1)}%`} style={{ stopColor: color, stopOpacity: 1 }} />
            ))}
          </linearGradient>
          <linearGradient id="arrow" gradientTransform="rotate(90)">
            {colors.arrow && colors.arrow.map((color, index) => (
              <stop key={`arr-${index}`} offset={index} style={{ stopColor: color }}/>
            ))}
          </linearGradient>
          <filter id="glow">
            <feDropShadow
        ...
        { score && score > 0 && (
          <>
            <circle cx="16" cy="13" r="10" fill="none" 
              ...
              strokeDashoffset={-1 * Math.PI * 10}
              strokeLinecap="round" />
            <polygon points="16,15 14,13 16,2 18,13" 
