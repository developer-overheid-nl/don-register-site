...
export function getDate(object: Partial<Record<status, any>>) {
  ...
  return date && new Date(date).toLocaleDateString('nl-NL', { year: 'numeric', month: 'long', day: 'numeric' });
}

export default function getAppearance(object?: Partial<Record<'status' | status, any>>) {
  if (!object) return;
  ...
  statuses.forEach(status => {
    if (keys.includes(status) && object[status]) {
      ...
      appearance = statuses.indexOf(status) > statuses.indexOf(object.status) ? status : object.status;
    }
  });
